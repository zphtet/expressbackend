...
# Prisma Seed Script for Cloud SQL PostgreSQL
# This script seeds initial test data to Cloud SQL from your local machine
...
echo -e "${BLUE}🌱 Prisma Seed to Cloud SQL${NC}"
echo "================================"
echo ""
...
if ! gcloud sql databases describe ${DATABASE_NAME} --instance=${INSTANCE_NAME} &> /dev/null; then
    echo -e "${RED}❌ Error: Database '${DATABASE_NAME}' not found${NC}"
    echo "Please create the database first or run migrations"
    exit 1
else
...
echo "Host: ${PUBLIC_IP}"
echo ""
echo -e "${YELLOW}📊 Seeded Data:${NC}"
echo "- 10 users (user1@example.com to user10@example.com)"
echo "- 10 posts (linked to users)"
echo "- 10 books (with sample titles)"
echo ""
echo -e "${YELLOW}⚠️  Note:${NC}"
echo "The seed script clears existing data before seeding."
echo "To keep existing data, modify seed.ts to skip the deleteMany() calls."
echo ""
